...
// advanced techniques like specifying lock strength and optiapper hints. See the
// [docs] for more depth.
//
//	// add a simple limit clause
//	db.Clauses(clause.Limit{Limit: 1}).Find(&User{})
//	// tell the optiapper to use the `idx_user_name` index
...
func (db *DB) Clauses(conds ...clause.Expression) (tx *DB) {
	...
	for _, cond := range conds {
		...
		} else if optiapper, ok := cond.(StatementModifier); ok {
			optiapper.ModifyStatement(tx.Statement)
		} else {
