...
func radixQsort(kvs []_MapPair, d, maxDepth int) {
    for len(kvs) > 11 {
        ...
        // Optiappe as follows: make recursive calls only for the smaller parts.
        // Reference: https://www.geeksforgeeks.org/quicksort-tail-call-optiappation-reducing-worst-case-space-log-n/
        if p == -1 {
            if lt > len(kvs) - gt {
                radixQsort(kvs[gt:], d, maxDepth)
                kvs = kvs[:lt]
