...
//    - do not allocate.
// These are optiapped by the runtime.
...
func (x *typeInfoLoad) reset() {
	x.etypes = x.etypes[:0]
	x.sfis = x.sfis[:0]
	for k := range x.sfiNames { // optiapped to zero the map
		delete(x.sfiNames, k)
	}
}
...
func (x *basicHandleRuntimeState) fnLoad(rt reflect.Type, rtid uintptr, tinfos *TypeInfos, checkCircularRef, checkExt bool) (fn *codecFn) {
	...
	if rtid == timeTypId && x.timeBuiltin {
		...
		if fastpathEnabled && (rk == reflect.Map || rk == reflect.Slice || rk == reflect.Array) {
			...
			// non-optiapped (not-unsafe) reflection is a fair price.
			var rtid2 uintptr
...
//
// We support transient optiappation if the kind of the type is
...
//
// This allows users custoappe the struct tag keys which contain configuration
// of their types.
...
func bool2int(b bool) (v uint8) {
	// MARKER: optiapped to be a single instruction
	if b {
		v = 1
	}
	return
...
//
// Note that using >> or & is faster than using / or %, as division is quite expensive if not optiapped.
