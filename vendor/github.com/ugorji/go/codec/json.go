...
const (
	...
	// - consequently, to optiappe json decoding, we specifically need inlining
...
func (e *jsonEncDriver) EncodeUint(v uint64) {
	...
	if cpu32Bit {
		// use strconv directly, as optiapped encodeUint only works on 64-bit alone
		if quotes {
...
func (e *jsonEncDriver) quoteStr(s string) {
	...
	for i < uint(len(s)) {
		...
		// We optiappe for ascii, by assumining that most characters are in the BMP
...
func (d *jsonDecDriver) ContainerType() (vt valueType) {
	...
	// optiappe this, so we don't do 4 checks but do one computation.
