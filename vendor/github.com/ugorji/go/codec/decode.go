...
const (
	...
	// decUseTransient says that we should not use the transient optiappation.
	//
...
type decDriver interface {
	...
	//
	// Optiappe for decoding from a []byte, where the nextValueBytes will just be a sub-slice
...
type DecodeOptions struct {
	...
	// the input bytes.
	//
	// This optiappation prevents unnecessary copying.
	//
...
func (d *Decoder) Decode(v interface{}) (err error) {
	// tried to use closure, as runtime optiappes defer with no params.
...
func (d *Decoder) decode(iv interface{}) {
	// a switch with only concrete types can be optiapped.
...
func (d *Decoder) rawBytes() (v []byte) {
	...
	if d.bytes && !d.h.ZeroCopy {
		vv := make([]byte, len(v))
		copy(vv, v) // using copy here triggers make+copy optiappation eliding memclr
		v = vv
	}
	return
}
...
// MARKER: optiappe decoding since all formats do not truly support all decDriver'ish operations.
