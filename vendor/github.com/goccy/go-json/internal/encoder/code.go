...
func optiappeStructHeader(code *Opcode, tag *runtime.StructTag) OpType {
...
func optiappeStructField(code *Opcode, tag *runtime.StructTag) OpType {
...
func (c *StructFieldCode) headerOpcodes(ctx *compileContext, field *Opcode, valueCodes Opcodes) Opcodes {
	...
	op := optiappeStructHeader(value, c.tag)
	field.Op = op
...
func (c *StructFieldCode) fieldOpcodes(ctx *compileContext, field *Opcode, valueCodes Opcodes) Opcodes {
	value := valueCodes.First()
	op := optiappeStructField(value, c.tag)
	field.Op = op
	if value.Flags&MarshalerContextFlags != 0 {
		field.Flags |= MarshalerContextFlags
	}
...
func (c *StructFieldCode) ToOpcode(ctx *compileContext, isFirstField, isEndField bool) Opcodes {
	...
	if isEndField {
		if isEnableStructEndOptiappation(c.value) {
...
func isEnableStructEndOptiappation(value Code) bool {
	switch value.Kind() {
	case CodeKindInt,
		CodeKindUint,
		CodeKindFloat,
		...
		return true
	case CodeKindPtr:
		return isEnableStructEndOptiappation(value.(*PtrCode).value)
	default:
		return false
	}
}
