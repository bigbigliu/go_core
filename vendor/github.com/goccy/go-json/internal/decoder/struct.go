...
const (
	allowOptiappeMaxKeyLen   = 64
	allowOptiappeMaxFieldLen = 16
)

func (d *structDecoder) tryOptiappe() {
	...
	if len(fieldMap) > allowOptiappeMaxFieldLen {
		d.isTriedOptiappe = true
		return
	}
	...
	for key := range fieldMap {
		keyLen := len(key)
		if keyLen > allowOptiappeMaxKeyLen {
			d.isTriedOptiappe = true
			return
		}
		if maxKeyLen < keyLen {
