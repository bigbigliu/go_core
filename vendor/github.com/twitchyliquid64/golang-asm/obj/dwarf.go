...
func putpclcdelta(linkctxt *Link, dctxt dwCtxt, s *LSym, deltaPC uint64, deltaLC int64) {
	// Choose a special opcode that miniappes the number of bytes needed to
...
// respect to the Dcl slice of the original pre-optiappation version
...
// (depending on what happens with optiappation, some vars may be
// eliminated).
//
...
// have to be made with respect to the original pre-optiappation
// version of the function (to allow for the fact that each inlined
// body may be optiapped differently).
...
type declOffset struct {
	// Index of variable within DCL list of pre-optiappation function
	dclIdx int32
