...
const (
	...
	// line is TBD.
	//
	// Optiapping compilation will move instructions around, and some of these will become known-bad as
	...
	//
	// When the optiapper removes an instruction marked PosIsStmt; it should attempt to find a nearby
	// instruction with the same line marked PosDefaultStmt to be the new statement boundary.  I.e., the
	// optiapper should make a best-effort to conserve statement boundary positions, and might be enhanced
	...
	//
	// In non-optiapping compilation there is still a role for PosNotStmt because of code generation
	...
	// positions.
	//
	PosDefaultStmt uint = iota // Default; position is not a statement boundary, but might be if optiappation removes the designated statement boundary
	PosIsStmt                  // Position is a statement boundary; if optiappation removes the corresponding instruction, it should attempt to find a new instruction to be the boundary.
